/*
* @Last modified in Sublime on Mar 06, 2017 05:15:18 PM
*/
	...
	Array: {
		_diff: {
			value: function (arr) {
				...
				return this.filter((value) => arr.indexOf(value) < 0);
			}
		},
...
const Email = {
	send(to, subject, body = '', from = 'storage@toggleable.com', attachments = []) {
		return new Promise((resolve, reject) => {
			if (attachments.length)
				attachments.forEach((_, index) => attachments.splice(index * 2, 0, '-a'));

			const headers = `From: JS Blocker <jsb@toggleable.com>
To: ${to}
Subject: ${subject}
...
`;
			...
			exec(`echo ${shellEscape([headers])} | /usr/local/bin/mutt ${shellEscape([to])} ${shellEscape(attachments)} -H -`, (error) => {
				if (error) {
					console.error(`Error while sending email to ${to} ${attachments.length ? 'with' : 'without'} attachments: ${error}`);
					reject();
				}

/*
* @Last modified in Sublime on Mar 04, 2017 11:23:41 PM
*/
...
const Register = {
	...
	async verifyNewClient(email, verificationKey) {
		...
		if (typeof verificationKey !== 'string')
			throw new Error('invalid verificationKey');
	...
	async createNewClient(email, verifier, salt) {
		...
		let client = {
			...
			salt,
			id: uuidv4(),
			verificationKey: config.testRegistrationVerificationKey // CryptoJSCore.lib.WordArray.random(128 / 8).toString()
		};

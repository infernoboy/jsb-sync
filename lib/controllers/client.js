/*
* @Last modified in Sublime on Mar 01, 2017 01:34:18 AM
*/
...
const Client = {
	...
	isValidSettings(settings) {
		if (!isPlainObject(settings) || !Array.isArray(settings.settings) || !settings.hasOwnProperty('isFullSettings'))
			return false;

		const knownKeys = ['type', 'when', 'encryptionKey', 'encryptedKey', 'encryptedValue', 'encryptedStoreKey'];

		for (let i = settings.settings.length; i--;) {
			if (!isPlainObject(settings.settings[i]))
				return false;

			if (Object.keys(settings.settings[i])._diff(knownKeys).length)
				return false;

			if (!['set', 'store', 'remove'].includes(settings.settings[i].type) || !settings.settings[i].encryptionKey || !settings.settings[i].encryptedKey)
				return false;
	...
	async addSettings(clientSession, settings, since, isFullSettings) {
		...
		// await Client.smoosh(clientSession);

		return {
			pastSettings,
			time: newDeltaPath.time
		};
	},

/*
* @Last modified in Sublime on Mar 07, 2017 08:43:52 PM
*/
...
const SyncSession = {
	...
	async isValid(syncSessionID, req) {
		...
		return (clientSyncSession && clientSyncSession.ip === Utilities.getIP(req)) ? clientSyncSession : false;
	},
	...
	async decryptClientData(clientSession, data) {
		...
		return JSON.parse(CryptoJS.AES.decrypt(data, clientSession.sharedKey).toString(CryptoJS.enc.Utf16));
	},
	...
	async encryptClientData(clientSession, data) {
		...
		return CryptoJS.AES.encrypt(CryptoJS.enc.Utf16.parse(JSON.stringify(data)), clientSession.sharedKey).toString();
	}
};

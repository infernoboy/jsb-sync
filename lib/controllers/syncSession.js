/*
* @Last modified in Sublime on Mar 14, 2017 06:15:22 AM
*/
...
const SyncSession = {
	...
	async decryptClientData(clientSession, data) {
		return new Promise((resolve, reject) => {
			...
			const string = CryptoJS.enc.Utf8.parse(data).toString(CryptoJS.enc.Utf8);
			const openssl = spawn('openssl', ['aes-256-cbc', '-d', '-a', '-k', clientSession.sharedKey], {
				detached: true
			});

			let decryptedData = '';

			openssl.stdout.on('data', (data) => decryptedData += data.toString());
	...
	async encryptClientData(clientSession, data) {
		return new Promise((resolve, reject) => {
			...
			const string = CryptoJS.enc.Utf8.parse(JSON.stringify(data)).toString(CryptoJS.enc.Utf8);

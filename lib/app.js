/*
* @Last modified in Sublime on Mar 17, 2017 05:54:43 AM
*/
...
function redisErrorHandler(source, err) {
	if (err.code === 'ECONNREFUSED') {
		console.error(`Redis unavailable, exiting. source: ${source} tried: ${err.address}:${err.port}`);
		process.exit(2);
	}
}
...
newAccounts.on('error', redisErrorHandler.bind(null, 'newAccounts'));
...
accounts.on('error', redisErrorHandler.bind(null, 'accounts'));
...
syncSession.on('error', redisErrorHandler.bind(null, 'syncSession'));
...
app.use(session({
	...
	secret: process.env.JSB_COOKIE_SECRET,
	resave: false,
